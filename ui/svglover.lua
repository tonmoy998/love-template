	...
	for i, svg in ipairs(svglover.onscreen_svgs) do
		...
		if svg.border_color ~= nil then
			love.graphics.setColor(
				svg.border_color[1] / 255,
				svg.border_color[2] / 255,
				svg.border_color[3] / 255,
				svg.border_color[4] / 255
			)
			love.graphics.rectangle(
				"fill",
				svg.region_origin_x - svg.border_width,
				svg.region_origin_y - svg.border_width,
				svg.region_width + svg.border_width * 2,
				svg.region_height + svg.border_width * 2
			)
...
function svglover._colorparse(str, default_r, default_g, default_b, default_a)
	...
	elseif string.match(str, "rgba%(%s*%d+%s*,%s*%d+%s*,%s*%d+%s*,%s*[^%)%+s]+%s*%)") then
		local red, green, blue, alpha =
			string.match(str, "rgba%(%s*(%d+)%s*,%s*(%d+)%s*,%s*(%d+)%s*,%s*([^%)%s]+)%s*%)")
		red = tonumber(red) / 255
		green = tonumber(green) / 255
		blue = tonumber(blue) / 255
...
function svglover._parsetransform(transform, extdata)
	...
	for cmd, strargs in string.gmatch(transform, "%s*(.-)%s*%((.-)%)") do
		...
		elseif cmd == "matrix" then
			...
			matrix:setMatrix(a, c, e, 0, b, d, f, 0, 0, 0, 1, 0, 0, 0, 0, 1)
...
function svglover._lineparse(state, line, extdata, options)
	...
	if
		string.match(line, "</?svg")
		or string.match(line, "<.xml")
		or string.match(line, "<!--")
		or string.match(line, "</?title")
		or string.match(line, "<!DOCTYPE")
	then
		-- ignore
	...
	if f_red ~= nil and vertexcount >= 6 then
		if options.use_love_fill == true then
			result = result
				...
				.. f_blue
				.. ","
				.. (f_alpha * f_opacity * opacity)
				.. ")\n"
				.. 'love.graphics.polygon("fill", extdata['
				.. bufferid
				.. "])"
		else
			...
			local stencil_fn = "local extdata = ...\n"
				.. 'return function() love.graphics.polygon("fill", extdata['
				.. bufferid
				.. "]) end\n"
